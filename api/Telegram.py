...
class Telegram(ObjectAPI, ObjectDb):
    ...
    def get_items_telegramm(self, params, trinfo=None):
        ...
        for row in cur.fetchall():
            ...
            data = "{0}::{1}::{2}::{3}::{4}::{5}".format(
                trinfo[0], trinfo[1], row[0], row[2], row[3], row[4]
            )
    ...
    def add_transaction(self, data):
        ...
        """.format(id_acc, id_item, id_user, amount, comment)
        try:
            cur = self.connect.cursor()
            cur.execute(sql)
            self.connect.commit()
            return "Сохранено! {0} - {1}".format(amount/100, comment)
        except:
